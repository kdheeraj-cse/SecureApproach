...
public class Register implements ActionListener
{
	...
	JLabel lab1;
	JComboBox cb;
	Map<String, String> cloudMap = new HashMap<String, String>(); //todelete start
	
	{
		cloudMap.put("CS1", "CloudServer");    
		cloudMap.put("CS2", "CloudServer2");
		cloudMap.put("CS3", "CloudServer3");
	}
	//todelete end
	
	public Register() 
	{
		frame = new JFrame("Register::An Approach For Content Delivery In Cloud");
		panel = new JPanel();
		...
		if(e.getSource()==submit)
		{
			...
			try{
				Socket skt = new Socket("localhost",2014);
				 ...
				 {
					 new File(cloudMap.get(cloudname)+"/"+name).mkdir(); //to delte
					 JOptionPane.showMessageDialog(null, "Registration Successfull for "+name);
					 JOptionPane.showMessageDialog(null, "Note Down your Password "+dis.readUTF());
					 frame.dispose();
				 }
